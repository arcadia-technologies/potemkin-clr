(ns flybot.ports.potemkin.types
  (:use
    [clojure [set :only (union)]]
    [flybot.ports.potemkin.macros :only (equivalent? normalize-gensyms safe-resolve unify-gensyms)])
  (:require
   [flybot.ports.riddley.walk :as r]
   [clojure.set :as set]
  ...
  (let [version (let [{:keys [major minor incremental ]} *clojure-version*]
    ...
    (remove
      #(when-let [min-version (-> % meta :min-version)]
         (neg? (#?(:clj .compareTo
                   :cljr compare) version min-version)))
      x)))
...
(defmacro definterface+
  ...
  (let [fn-names (map first body)
    ...
    `(let [p# ~(if (try
                     (#?(:clj Class/forName
                         :cljr clojure.lang.RT/classForName) class-name)
                     true
                     (catch Exception _
                       false))
...
(defmacro deftype+
  ...
  [name params & body]
  (let [body (->> (list* 'deftype name params 'flybot.ports.potemkin.types.PotemkinType body)
               clean-deftype
               expand-deftype
               deftype*->deftype)
...
(defmacro reify+
  ...
  [& body]
  (let [body (->> (list* 'deftype (gensym "reify") [] 'flybot.ports.potemkin.types.PotemkinType body)
               clean-deftype
               expand-deftype
               deftype*->deftype)]
